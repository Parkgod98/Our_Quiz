...
같은 스터디 안에서는 학습 진행을 서로 확인할 수 있도록 문제집별 점수, 전체 문제 수, 정답 수, 오답 수 같은 요약 정보만 멤버 간 공유합니다. 다른 멤버의 개별 제출 답안과 정답 상세는 공유하지 않고, 본인 결과 상세만 조회할 수 있게 유지합니다.
...
- 그룹 기록용 Security Definer 함수는 현재 사용자가 해당 그룹의 멤버인지 먼저 확인한 뒤 점수 요약만 반환합니다.
...
결과 상세에서 정답을 내려주는 RPC는 제출 완료된 본인 Attempt에만 허용합니다. 다른 스터디 멤버에게는 점수/오답 수만 공유하고 정답 및 제출 답안은 노출하지 않습니다.
