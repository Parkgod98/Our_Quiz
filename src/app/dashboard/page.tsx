...
type AssignmentRow = { group_id: string; version_id: string; assigned_at: string; question_set_versions: { question_count: number; question_sets: { title: string; week_number: number } } };
...
export default async function DashboardPage() {
  ...
  const [attemptResult, answerResult, membershipResult, profileResult] = await Promise.all([
  ...
  const assignmentsResult = groupIds.length > 0 ? await supabase.from("group_question_sets").select("group_id,version_id,assigned_at,question_set_versions!inner(question_count,question_sets!inner(title,week_number))").in("group_id", groupIds).order("assigned_at", { ascending: false }).limit(8) : { data: [] as AssignmentRow[] };
